...
learning_rate = 0.001
save_path = './logs'
subset_size = 10000  # 選取 CIFAR-10 子集的大小
...
subset = Subset(dataset, list(indices))  # Convert indices to a list
...
model = PixelCNN(n_channel=3, h=128, layers=15, feature_maps=1024).to(device)
...
best_loss = float('inf')
best_ckpt_path = os.path.join(save_path, 'best_cifar10.ckpt')
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        for data, target in val_iterator:
            ...
            output = model(data)                                                    # [batch_size, 256, 1, 32, 32]
            
            # sample
            probs = torch.softmax(output, dim = 1).data                             # [batch_size, 256, 1, 32, 32]
            output_tensor = torch.argmax(probs, dim=1)/ 255.                        # [batch_size, 1, 32, 32]
    ...
    if avg_val_loss < best_loss:
        ...
        torch.save(model.state_dict(), os.path.join(save_path, 'best_cifar10.ckpt'))
    ...
    with torch.no_grad():
        for i in range(20, 28):                                        # generate bottom half
            for j in range(28):
                out = model(data)                                      # [batch_size, 256, 1, 32, 32]
